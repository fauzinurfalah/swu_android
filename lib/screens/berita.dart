...
class _BeritaState extends State<Berita> {
  ...
  static const String _readPrefsKey = 'berita_read_ids';
  static const String _starPrefsKey = 'berita_starred_ids';
  ...
  Future<void> _initPrefsAndData() async {
    ...
    // load data read & starred dari SharedPreferences
    final readList = _prefs!.getStringList(_readPrefsKey) ?? [];
    final starList = _prefs!.getStringList(_starPrefsKey) ?? [];

    setState(() {
      _readSet.addAll(readList);
      _starredSet.addAll(starList);
    });

    await _getBeritaAkademik();
  }
  ...
  String _keyForBerita(dynamic berita, int index) {
    if (berita is Map && berita["id"] != null) {
      return "id_${berita["id"]}";
    }
    if (berita is Map && berita["slug"] != null) {
      return "slug_${berita["slug"]}";
    }
    return "idx_$index"; // fallback
  }

  Future<void> _saveReadState() async {
    if (_prefs == null) return;
    await _prefs!.setStringList(_readPrefsKey, _readSet.toList());
  }

  Future<void> _saveStarState() async {
    if (_prefs == null) return;
    await _prefs!.setStringList(_starPrefsKey, _starredSet.toList());
  }

  void _toggleStar(String key) {
    setState(() {
      if (_starredSet.contains(key)) {
        _starredSet.remove(key);
      } else {
        _starredSet.add(key);
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < beritaAkademik.length; i++) {
      ...
      final isRead = _readSet.contains(key) ||
          (b is Map && (b['is_read'] == true || b['read'] == true));
      ...
      switch (selectedFilter) {
        ...
        case BeritaFilter.starred:
          include = _starredSet.contains(key) ||
              (b is Map &&
                  (b['starred'] == true || b['is_starred'] == true));
          break;
      }
                    ...
                    itemBuilder: (context, index) {
                      ...
                      Widget starButton(String key) => GestureDetector(
                            ...
                            child: Container(
                              ...
                              height: 44,
                              decoration: BoxDecoration(
                                color: _starredSet.contains(key)
                                    ? Colors.amber
                                    : Colors.white,
                                shape: BoxShape.circle,
                                border: Border.all(
                      ...
                      final bool isStarredNow = _starredSet.contains(key) ||
                          (berita is Map &&
                              (berita['starred'] == true ||
                                  berita['is_starred'] == true));
                            ...
                            GestureDetector(
                              onTap: () {
                                ...
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => DetailBeritaPages(
                                      berita: mapBerita,
                                      isStarred: isStarredNow,
                                    ),
                                  ),
                                );
                              },
